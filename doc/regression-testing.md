...
- `swift test`: <!-- counts:swift-tests -->971 Swift Testing tests<!-- counts:end --> with no known-issue wrappers, using the real Apple
...
- `python3 -m unittest discover -s tools -p 'test_*.py' -v`: <!-- counts:python-tests -->258 Python tests<!-- counts:end --> over the tools,
...
**The results directory is kept only while it is worth reading.** A run where every gate passed
removes its own directory at the end and says so; a run with a failure or an unmeasured ceiling
keeps it and prints the path, which is what the path is for. Each run also prunes all but the five
most recent directories left behind by earlier runs, naming each one it removes — set
...
<!-- counts:contract-coverage -->1529 checks on 271 reviewed pages across 24 documents<!-- counts:end -->.
...
Those 1529 checks are 4 `spineContinuity`, 127 `text`, 807 `orderedText`, 71 `absentText`,
52 `headings`, 154 `paragraphs`, 16 `continuedParagraphs`, 10 `preformatted`,
6 `absentPreformatted`, 15 `lists`, 7 `asides`, 2 `quotations`, 16 `scripts`, 11 `math`,
11 `imageRegions`, 37 `tableRows`, 124 `minimumImages`, 1 `originalPageImage`,
29 `warningCodesAnyOf` and 29 `absentWarningCodes`, counted as `tools/check_corpus_content.py`
counts them.
<!-- counts:end -->
...
printed number is pinned — must hold the phrases in consecutive items, in order, so items that
split into separate lists, lose their numbering, change order or fall back to paragraphs or
preformatted blocks fail; a list is on every page one of its items opens on, and reading an EPUB
refuses a list element holding anything but items, #292),
...
misplaced scripts, changed MathML structure or missing fallback, missing or wrong-page warnings,
changed source identity, failed conversion,
