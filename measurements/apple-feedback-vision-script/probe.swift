...
let mode = ProcessInfo.processInfo.environment["MODE"] ?? "library"
request.textRecognitionOptions.useLanguageCorrection = (mode == "correct" || mode == "correct+en")
if mode == "en" || mode == "correct+en" || mode == "library" {
    let english = Locale.Language(identifier: "en")
    if request.supportedRecognitionLanguages.contains(english) {
        request.textRecognitionOptions.recognitionLanguages = [english]
    }
}
FileHandle.standardError.write(Data("mode=\(mode) correction=\(request.textRecognitionOptions.useLanguageCorrection) languages=\(request.textRecognitionOptions.recognitionLanguages)\n".utf8))
