#!/usr/bin/env python3
"""usage: case.py <case-id> [base] [cand] [compare]  lanes for one corpus case, compare, keep both EPUBs for page review.

The work directory (ALIGNED_TABLES_WORK) holds bin/base/pdf-reflow, bin/cand/pdf-reflow and bin/probe;
both converters run as `pdf-reflow` so they share one Vision model cache name. Lane outputs are written
there; after `compare` both EPUBs are copied to review/ and everything else is deleted. With no labels:
base, cand, then compare. Stops when under 8 GB are free."""
import json, os, shutil, subprocess, sys, time
from pathlib import Path
S = os.environ['ALIGNED_TABLES_WORK']
W = str(Path(__file__).resolve().parents[3])
KEEP = W + '/measurements/aligned-column-tables/lane-summaries'
case = sys.argv[1]
...
for label in [l for l in labels if l != "compare"]:
    ...
    with open(out + '.log', 'w') as log:
        rc = subprocess.run(['python3', 'tools/run_corpus_regressions.py', '--converter', f'{S}/bin/{label}/pdf-reflow',
...
os.makedirs(f'{S}/review', exist_ok=True)
for label in ('base', 'cand'):
    epubs = [os.path.join(d, f) for d, _, fs in os.walk(f'{S}/lane/{label}-{case}') for f in fs if f.endswith('.epub')]
    for path in epubs:
        shutil.copy(path, f'{S}/review/{label}-{case}.epub')
    shutil.rmtree(f'{S}/lane/{label}-{case}', ignore_errors=True)
    os.remove(f'{S}/lane/{label}-{case}.log')
shutil.rmtree(cmp, ignore_errors=True)
