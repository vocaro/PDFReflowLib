...
// rectangle, so an answer key's numbering sequence can be read off the order. A crop is followed by
// the `N)` numbers printed on the lines it covers (#185). Build it from the
        ...
        for number in pages {
            try autoreleasepool {
                ...
                let elements = free.map { LayoutReconstructor.Element(rect: $0.readingRect ?? $0.rect, line: $0) }
                    + regions.enumerated().map {
                        LayoutReconstructor.Element(rect: $0.element, image: "region-\($0.offset)",
                            held: LayoutReconstructor.heldLines(of: $0.element, in: content.lines, among: regions))
                    }
                ...
                for (index, element) in LayoutReconstructor.ordered(elements, bodySize: body).enumerated() {
                    let r = element.rect
                    let numbers = LayoutReconstructor.numberedMarkers([element]).map(\.number)
                    let what = element.line.map { "\"\($0.text)\"" }
                        ?? ((element.image ?? "?") + (numbers.isEmpty ? "" : " holding \(numbers)"))
