...
func magazineCouponOutlineBoxesAreReadableMarks() throws {
    ...
    #expect(reading.rows.allSatisfy { $0.box.minX > 218 && $0.box.maxX < 234 })
    #expect(reading.rows.map { content.lines[$0.labelIndex].text }.sorted()
        == ["To change your address", "To stop mailing"])
...
func ruledLabelCheckboxesReachTheEPUBAsText() async throws {
    ...
    #expect(extracted.content.lines.filter { $0.text.hasSuffix(" ☐") }.map(\.text).sorted()
        == ["To change address ☐", "To stop mailing ☐"])
