...
- `swift test`: <!-- counts:swift-tests -->838 Swift Testing tests<!-- counts:end --> with no known-issue wrappers, using the real Apple
...
<!-- counts:contract-coverage -->1329 checks on 238 reviewed pages across 21 documents<!-- counts:end -->.
...
Those 1329 checks are 4 `spineContinuity`, 117 `text`, 670 `orderedText`, 71 `absentText`,
51 `headings`, 143 `paragraphs`, 6 `continuedParagraphs`, 10 `preformatted`, 15 `lists`, 7 `asides`,
2 `quotations`, 16 `scripts`, 11 `imageRegions`, 37 `tableRows`, 121 `minimumImages`,
