...
| Full-page scans | `.jpeg(quality: 0.90)` for tinted/noisy scans when lossy encoding is acceptable; compare PNG for clean black-and-white scans | Try 0.75–0.90 as a measured scan tuning range. The [17-page sweep](../measurements/raster-device-qualification/record.md) tests JPEG 0.60/0.75/0.85/0.90/0.95 across eleven documents; the encoded-image phrase proxy can change, and 0.60 shows visible edge artifacts on colored labels. Production OCR reads the raster before output encoding. Clean tables can be smaller as PNG. This is a scan recommendation, not a range for all artwork. |
...
| Resolution | Start at 180 DPI; compare 240 DPI for dense small print or fine diagram labels | The [current 17-page sweep](../measurements/raster-device-qualification/record.md) measures 96/120/150/180/240/300 DPI. PNG full-page bytes are about −63/−47/−27/+49/+93% relative to 180. Tiny Warren notes recover 0/0/3/5/6/8 of nine reviewed phrases across those settings. Try 150 only for a storage-constrained body-text scan whose small notes are not essential; 96/120 are not general small-print recommendations. Actual resolution can be reduced by the pixel ceiling. |
| Raster pixel ceiling | Keep the 12-million-pixel starting bound while profiling the target device | The [physical-device record](../measurements/raster-device-qualification/record.md#ipad-results) measures binding 6/12/24/48-million-pixel ceilings on an iPhone 17 Pro Max and an iPad Air 13-inch (M3), with full-book memory measured separately. At 48M, sampled footprint reaches about 648/719 MiB respectively. The supported 1–48 million range is not a universal safe-device recommendation, and this per-raster ceiling is not an app-wide RAM cap. |
...
[broader raster qualification](../measurements/raster-device-qualification/record.md) records
JPEG below 0.90, isolated pages at 96–300 DPI and current whole-book/OCR effects. Its named
hardware, source coverage and retained failures bound the recommendations; they do not establish
a universal safe-device range or OCR accuracy guarantee.
