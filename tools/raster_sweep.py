#!/usr/bin/env python3
"""Sweep raster DPI and JPEG quality over reviewed corpus pages with the production rasterizer.

The sweep compiles `tools/probe-raster-sweep.swift` against the actual `PageRasterizer` and
value types, then runs one fresh process per DPI. Each process renders every target's page and
reviewed region, encodes the same CGImage as PNG and as JPEG at each quality, and recognizes the
encoded region files with Vision. For every image this driver records bytes, pixels, effective
DPI, the JPEG's error against its same-DPI PNG, the corpus checkers' metrics against reviewed
references (raw, as the corpus gate would see the image, and after resampling to the 180 DPI
the references assume), and the fraction of reviewed phrases Vision finds. Per-process peak RSS
and physical footprint come from `/usr/bin/time -l`.

Targets are reviewed source pages/regions, chosen from the source, never from converter output.
References are the committed corpus references or measurement-only references rendered with
`tools/render_region_reference.py --output-root`. This measures rasters of isolated pages; it is
not a whole-book conversion, an OCR accuracy rate, a reader study or a device budget.
"""
import argparse
...
BASE_DPI = image_regions.RENDER_DPI
SOURCE_DPI = 360
SHEET_WIDTH = 1400
...
def encoding_label(quality):
    ...
    return 'png' if quality is None else f'jpeg{round(quality * 100)}'
...
def normalized_to_base(image, effective_dpi):
    """Resample to the reference resolution (Lanczos) so 180-DPI references apply at other DPIs."""
    factor = BASE_DPI / effective_dpi
...
def load_reference(root, relative, case, page):
    """Validate a reference path against the target's source and page; return its sidecar and PNG bytes."""
    path = Path(root) / relative
    sidecar = json.loads(path.with_suffix('.json').read_text())
    if sidecar.get('sourceSHA256') != case['sha256'] or sidecar.get('page') != page:
        raise ValueError(f'reference {relative} was rendered from another source or page')
    if sidecar.get('renderDPI') != BASE_DPI:
        raise ValueError(f'reference {relative} uses another render resolution')
    kind = sidecar.get('kind', 'region')
    if kind not in ('region', 'glyph', 'color'):
        raise ValueError(f'reference {relative} has unknown kind {kind}')
    return {'reference': str(relative), 'kind': kind, 'regionPoints': sidecar.get('regionPoints'),
            'sha256': sidecar.get('sha256'), 'data': path.read_bytes()}


def load_targets(path, manifest, cache_dir, root=ROOT, pdfinfo='pdfinfo', box=crop_box):
    ...
    for target in targets:
        ...
        target['references'] = [load_reference(root, relative, case, target['page'])
                                for relative in target.get('references', [])]
    return targets
...
def reference_metrics(reference, image, region_points):
    """Checker metrics of one reference against one PIL image."""
    data = reference['data']
    if reference['kind'] == 'glyph':
        passed, metrics = glyph_structure.structure_score(data, [encoded(image)])
        return {'passed': passed, **{k: metrics[k] for k in ('coverage', 'extraInk', 'sharpness', 'weakestStroke', 'correlation')}}
    metrics = {}
    if reference['kind'] == 'region':
        metrics['correlation'] = round(image_regions.region_score(data, [encoded(image)]), 4)
        metrics['regionPassed'] = metrics['correlation'] >= image_regions.DEFAULT_MINIMUM_CORRELATION
    if region_points:
        passed, appearance = image_appearance.appearance_score(data, region_points, [encoded(image)])
        metrics.update({k: v for k, v in appearance.items() if k != 'correlation' or 'correlation' not in metrics},
                       appearancePassed=passed)
    return metrics
...
def fidelity(task):
    """Raw and 180-DPI-normalized checker metrics for one encoded image (a process-pool task)."""
    path, references, effective_dpi = task
    with Image.open(path) as image:
        image = image.convert('RGB')
        normalized = normalized_to_base(image, effective_dpi)
        results = []
        for reference in references:
            entry = {'reference': reference['reference'], 'kind': reference['kind'],
                     'raw': reference_metrics(reference, image, reference['regionPoints'])}
            entry['normalized'] = (entry['raw'] if normalized is image
                                   else reference_metrics(reference, normalized, reference['regionPoints']))
...
def summarize(rows, processes):
    ...
    for dpi, encoding in settings:
        ...
        coverage = [entry_['normalized']['coverage'] for row in subset for entry_ in row.get('fidelity', [])
                    if entry_['kind'] == 'glyph' and row['kind'] == 'region']
        entry['minimumGlyphCoverageNormalizedRegion'] = round(min(coverage), 4) if coverage else None
...
def main():
    ...
    tasks = [(output / row['relativeFile'], by_target[row['target']]['references'], row['effectiveDPIX'])
             for row in rows if by_target[row['target']]['references']]
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        results = dict(pool.map(fidelity, tasks, chunksize=4))
    for row in rows:
        key = str(output / row['relativeFile'])
        if key in results:
            row['fidelity'] = results[key]
    print(f'PASS fidelity: {len(tasks)} images checked', flush=True)
