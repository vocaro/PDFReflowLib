...
enum NativeTextReader {
    static func lines(on page: PDFPage, limit: Int, includeStyle: Bool = true) throws -> [TextLine] {
        ...
        for line in selection.selectionsByLine() {
            ...
            let text = raw.trimmingCharacters(in: mono ? .newlines : .whitespacesAndNewlines)
            var styled: InlineText?
            if let attributed, attributed.string.trimmingCharacters(in: .whitespacesAndNewlines) == text {
