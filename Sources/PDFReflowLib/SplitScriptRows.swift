...
/// The PDFKit lines that are pieces of one printed row of stacked scripts, read back as that row
/// (#303).
///
...
/// - **It carries on from scripts.** The row ends in script glyphs, or the piece is scripts alone
///   standing at two levels (DASC page 2's `N` with `f` under it, after `n`). A raised note number
///   after a body glyph is a single level and is left as PDFKit returned it.
...
///   rule, and the glyphs around it are not scripts. No mark at most 6 points wide and taller
///   than that stands up through the row as far as the piece reaches: that is a table's column
///   rule, and the two are cells.
///
...
enum SplitScriptRows {
    ...
    static func rejoin(_ items: [Item], marks: [CGRect]) -> [Item] {
        ...
        for start in items.indices where !claimed[start] && !scriptOnly[start] {
            ...
            while true {
                var next: (index: Int, delta: Double)?
                for index in max(0, start - window)...min(items.count - 1, row.last + window)
                where !claimed[index] && index != start && !row.members.contains(where: { $0.index == index }) {
                    // A line PDFKit returned earlier that is not a stray script has been read on its own.
                    guard index > start || scriptOnly[index], let piece = pieces[index],
                          let delta = continues(row, with: piece, bars: bars, posts: posts),
                          !interrupted(row, by: index) else { continue }
                    next = (index, delta)
                    break
                }
                guard let next, let piece = pieces[next.index] else { break }
                row.members.append(next)
                row.bounds = row.bounds.union(piece.bounds)
                row.last = max(row.last, next.index)
                row.take(piece.runs)
            }
    ...
    private static func continues(_ row: Row, with piece: Piece, bars: [CGRect], posts: [CGRect]) -> Double? {
        ...
        // Only a row of stacked scripts: the piece carries on from scripts that close the row, or
        // holds a stack of its own (DASC page 2's `N` with `f` under it, after `n`). A raised note
        // number after a body glyph, with or without the words that follow it, is a single level
        // and is left as PDFKit returned it.
        guard row.closingWidth > 0 || bodyRuns.isEmpty && standsAtTwoLevels(glyphs) else { return nil }
        ...
        if bars.contains(where: { $0.intersects(among) }) { return nil }
