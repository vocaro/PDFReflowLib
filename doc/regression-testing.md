...
- `swift test`: <!-- counts:swift-tests -->811 Swift Testing tests<!-- counts:end --> with no known-issue wrappers, using the real Apple
...
<!-- counts:contract-coverage -->985 checks on 210 reviewed pages across 21 documents<!-- counts:end -->.
...
Those 985 checks are 4 `spineContinuity`, 115 `text`, 374 `orderedText`, 71 `absentText`,
51 `headings`, 114 `paragraphs`, 6 `continuedParagraphs`, 10 `preformatted`, 15 `lists`, 7 `asides`,
