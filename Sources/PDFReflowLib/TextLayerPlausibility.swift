...
///   carry digits is a table or form and is not judged: the Blue Book's statistical tables read
///   below half English, and a fresh recognition of their handwritten cells reads no better
///   (`measurements/text-layer-plausibility/record.md`).
...
///   neighbour joins into an English word: `tcld t» ftboot` for "told me about" on Warren's carbon
///   typescripts, which read half to three quarters English. Text split inside words (`fi e ld stre
///   ngth`, the NBS paper) joins up and is not counted
///   (`measurements/suspect-text-layers/record.md`).
...
enum TextLayerPlausibility {
    ...
    static func wordCounts(_ text: String, isWord: (String) -> Bool) -> WordCounts {
        ...
        for (index, word) in words.enumerated() {
            ...
            // (`McDonald`, Warren's witness lists).
    ...
    /// and art it cannot read (Arabic and Cyrillic letters on Warren's handwritten hospital notes,
    /// Thai in the Blue Book's handwritten table cells, `НИН?!` for the CDC comic's `HUH?!`).
    ...
    /// pass it to become a heading, and so a navigation entry: under `--ocr always` the Blue Book's
    /// table cells (`139`, `1952 1950`, `Certaia Doubtlul Total`) and Warren's handwriting
    /// (`Tag De Praciy Glii tant ami She`) were read at heading size. True without a lexicon when no
    /// other script appears.
    ...
    /// the page has (the CDC comic's page 7 reads `powtred` and `radic`), while recognition of
    /// handwriting (Warren's hospital notes and letters) reads under half English. Such a reading
    /// is noise, which serves a reader worse than the page image.
    static func judgeRecognized(lines: [TextLine], language: String) -> Finding? {
        let text = lines.map(\.text).joined(separator: "\n")
        guard !lines.isEmpty, TextEncodingCheck.supports(language: language),
    ...
    /// Recognition of Warren's handwriting is named English, Polish, Czech or Swedish, mostly with
    /// less confidence than that (pages 549–556); page 548's is named Romanian with full confidence,
    /// the one such reading surveyed, and stands. The other languages' lexicons are not on every
    /// system (none but English's on the macOS 27 survey machine), so the words are not checked.
    ...
    static func recognitionMessage(_ finding: Finding, keepsCrops: Bool) -> String {
        ...
        return "OCR of this page image does not read as English: only \(english) of \(judged) words are English words "
            ...
            + (keepsCrops ? "the page's artwork is preserved as images and its writing does not reflow."
