    ...
    static func main() throws {
        ...
        for p in first...last {
            autoreleasepool {
                ...
                if let index = try? StructureTreeReader.read(URL(fileURLWithPath: args[1])), let tags = index.pages[p], !tags.isEmpty,
                   StructureTreeReader.validates(tags, owners: index.owners[p] ?? [:], page: ref) {
