...
- `swift test`: <!-- counts:swift-tests -->374 Swift Testing tests<!-- counts:end --> with no known-issue wrappers, using the real Apple
...
<!-- counts:contract-coverage -->515 checks on 120 reviewed pages across 18 documents<!-- counts:end -->.
...
Those 515 checks are 4 `spineContinuity`, 81 `text`, 184 `orderedText`, 42 `absentText`,
34 `headings`, 36 `paragraphs`, 6 `continuedParagraphs`, 16 `scripts`, 17 `imageRegions`,
