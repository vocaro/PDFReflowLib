#!/bin/bash
# usage: lane.sh <case-id> <binary-label: base|cand>  one corpus-lane case; outputs kept for compare.sh
T=$(cd "$(dirname "$0")" && pwd)
W=$(cd "$T/../../.." && pwd)
...
out=$S/lane/$2-$1
rm -rf "$out"; mkdir -p "$S/lane/keep"
cd "$W"
start=$(date +%s)
python3 tools/run_corpus_regressions.py --converter "$S/bin/${2/base/baseffc}" --epubcheck /opt/homebrew/bin/epubcheck \
...
EOF
cp "$out/summary.json" "$S/lane/keep/$2-$1.json"
