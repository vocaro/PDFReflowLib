#!/usr/bin/env python3
"""usage: case.py <case-id>  base and cand lanes for one corpus case, compare, page diffs, then delete outputs."""
import json, os, shutil, subprocess, sys, time
from pathlib import Path
# Work directory holding bin/{base,cand,probe}; lane outputs are written there and deleted.
S = os.environ['TABLE_CAPTIONS_WORK']
W = str(Path(__file__).resolve().parents[3])
KEEP = W + '/measurements/table-captions-and-tags/lane-summaries'
case = sys.argv[1]
labels = sys.argv[2:] or ['base', 'cand']
...
# Keep the EPUBs for hand review of changed pages: copy both into review/, delete everything else.
