...
- `swift test`: <!-- counts:swift-tests -->1056 Swift Testing tests<!-- counts:end --> with no known-issue wrappers, using the real Apple
...
<!-- counts:contract-coverage -->1608 checks on 289 reviewed pages across 24 documents<!-- counts:end -->.
...
Those 1608 checks are 4 `spineContinuity`, 127 `text`, 831 `orderedText`, 78 `absentText`,
65 `headings`, 154 `paragraphs`, 19 `continuedParagraphs`, 10 `preformatted`,
...
inside `<pre>` and nested emphasis), `absentScripts` (the mirror of it: no superscript or
subscript on the page holds exactly this text — what pins a number the page sets on the line,
#308), `math` (the exact ordered MathML content tree and
