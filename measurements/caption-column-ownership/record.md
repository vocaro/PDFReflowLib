...
Combined USDA verification: a full 24-page conversion with this caption follow-up and the separately reviewed planner correction `4bd1ca9` restored pages 8/10/13/15 to the first qualified candidate's exact text and paragraphs; `pyrethroids`, `neonicotinoids`, and `assay` all reflow correctly. Local converter SHA: `807ed89b6b9f717f85d435e08cd266a5f718d1be81bdd329c0bafb526e70404f`; EPUB SHA: `605b28cecdf3dafecb4a9654be002e25c0270f3be49e3dba2cfb27d437f989f2`. This local branch/run still lacked the page-16 initial A in both the narrowed-only and combined runs, while the root's integrated v2 retained it; the root's exact final run must retain its source-16 control. This comparison is not a replacement for final integrated qualification.

The NOAA 65 corpus control additionally requires the complete body paragraph to continue into physical page 66 (`Quality of life is also threatened by climate change` / `in ways that can be more difficult to quantify`). This detects a misplaced photo caption after the last body paragraph even when all within-page text remains present; verify it against the next integrated NOAA output.
