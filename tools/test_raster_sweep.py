...
class PhraseAndLabelTests(unittest.TestCase):
    ...
    def test_encoding_labels_match_the_probe(self):
        ...
        self.assertEqual(raster_sweep.encoding_label(0.605), 'jpeg61')  # Swift rounds ties away from zero.
...
class TargetLoadingTests(unittest.TestCase):
    ...
    def load(self, targets):
        ...
        return raster_sweep.load_targets(path, self.manifest, self.root / 'cache', box=lambda source, page, pdfinfo: (600.0, 800.0))

    def test_normalized_regions_become_points_without_legacy_checker_dependencies(self):
