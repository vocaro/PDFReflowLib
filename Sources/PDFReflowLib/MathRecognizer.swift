...
enum MathRecognizer {
    ...
    /// The page's glyphs from its content stream. Maths italic comes from the font's own name
    /// (`FontWeightReader.isMathItalic`). A show in a bold font is opaque: Wallace page 15 sets the
    ...
    static func glyphs(on page: CGPDFPage) -> PageGlyphs {
        var mathItalicFonts: Set<Int> = [], boldFonts: Set<Int> = []
        _ = FontWeightReader.read(page, fonts: { id, info in
            if info.mathItalic == true { mathItalicFonts.insert(id) }
            if info.weight == .bold { boldFonts.insert(id) }
        })
        var result = PageGlyphs()
        for show in NativeSpacingReader.read(page, recordingGlyphs: true) {
            guard let glyphs = show.glyphs, !boldFonts.contains(show.font) else {
    ...
    static func explains(_ glyphs: [Glyph], lines: [TextLine], crop: CGRect) -> Bool {
        ...
        for glyph in glyphs {
            for character in InlineText.spellingOutLigatures(glyph.text) where !character.isWhitespace {
